...
def rankFeature(url):
    ...
    aucList = [values1 for values, values1 in sorted(list(enumerate(auclist)), key=lambda x:x[1],reverse=True)]
    return rankList,aucList
...
def calNN(wronginstance,data,k1,k2,labels):
    ...
    return com
def calComplementarity(dataset):
    complementarity = np.zeros((100,100),dtype=float)
    for i in range(100):
        f1 = dataset[:,i]
        for j in range(100):
            com = 0
            ...
            if(i!=j):
                complementarity[i,j] = index * (com/len(w))
            else:
                complementarity[i,j] = 0
       # com[i][j] = c
    return complementarity
    print(complementarity)
def calCombine(alpha,complementarity,auc):
    combine = np.zeros((100,100))
    for i in range(100):
        for j in range(100):
            combine[i,j] = alpha * max(auc[i],auc[j]) + (1-alpha)*complementarity[i,j]
    return combine
url = "musk2.csv"
labels,dataset = loadData(url)
...
n = dataset.shape[0]
m = dataset.shape[1]
rl,auclist = rankFeature(url)[0:100]
newdataset = dataset[:,rl]
com = calComplementarity(newdataset)
#print(com)
combine = calCombine(0.6,com,auclist)
print(combine)
def selectFeat(combine,auclist):
    FeatList = []
    FeatList.append(1)
    for 
